...
describe("useQuizStore", () => {
  ...
  it("setQuizList를 통해 퀴즈 목록과 시작 날짜를 업데이트 해야 한다", () => {
    const { result } = renderHook(() => useQuizStore());
    act(() => {
      result.current.setQuizList(
  ...
  it("유저 답변을 추가하면 userAnswerList가 업데이트 되어야 한다", () => {
    ...
    act(() => {
      result.current.setUserAnswer(testAnswer);
    });
  ...
  it("스토어를 초기화하면 모든 데이터가 초기 상태로 복구되어야 한다", () => {
    ...
    // 상태를 초기화
    act(() => {
      result.current.setQuizList([], new Date(SECOND_DATE));
    });
